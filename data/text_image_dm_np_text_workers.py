...
from dataset.gen_img_train_list import get_img_path_from_txt
...
class TextImageDataset(Dataset):
    def __init__(self,
        ...
        path_text =Path(os.path.join(folder,"prompt_text"))
        path_img =Path(os.path.join(folder,"img_data"))
        ...
        with open(train_val_file, 'r') as file:
            ...
            train_list_img =get_img_path_from_txt(train_val_file)
            text_files =[Path(file) for file in train_list]
            image_files =[Path(file) for file in train_list_img]
    ...
    def __getitem__(self, ind):
        ...
        image_file_npz_path = self.image_files[key]
...
class TextImageDataModule(LightningDataModule):
    ...
    def setup(self, stage=None):
        self.dataset = TextImageDataset(self.folder,train_val_file='dataset/9w/train_set.txt', image_size=self.image_size, resize_ratio=self.resize_ratio, shuffle=self.shuffle, custom_tokenizer=not self.custom_tokenizer is None)
        self.dataset2 = TextImageDataset(self.folder,train_val_file='dataset/9w/val_set.txt', image_size=self.image_size,resize_ratio=self.resize_ratio,shuffle=self.shuffle,custom_tokenizer=not  self.custom_tokenizer is None)
